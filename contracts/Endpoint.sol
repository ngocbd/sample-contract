...
contract Endpoint is IEndpoint, EIP712Upgradeable, OwnableUpgradeable, Version {
    ...
    string constant BURN_LP_SIGNATURE = "BurnLp(bytes32 sender,uint32 productId,uint128 amount,uint64 nonce)";
    string constant LINK_SIGNER_SIGNATURE = "LinkSigner(bytes32 sender,bytes32 signer,uint64 nonce)";
    ...
    function initialize(
        ...
        slowModeConfig = SlowModeConfig({timeout: slowModeTimeout, txCount: 0, txUpTo: 0});
    ...
    function chargeFee(bytes32 sender, int128 fee, uint32 productId) internal {
        IProductEngine.ProductDelta[] memory deltas = IProductEngine.ProductDelta[](
    ...
    function validateSignature(bytes32 sender, bytes32 digest, bytes memory signature) internal view virtual {
        address recovered = ECDSA.recover(digest, signature);
        require(
            (recovered != address(0)) &&
                ((recovered == address(uint160(bytes20(sender)))) || (recovered == linkedSigners[sender])),
            ERR_INVALID_SIGNATURE
        );
    }

    function increaseAllowance(IERC20Base token, address to, uint256 amount) internal virtual {
    ...
    function safeTransferTo(IERC20Base token, address to, uint256 amount) internal virtual {
    ...
    function validateSender(bytes32 txSender, address sender) internal view {
        require(address(uint160(bytes20(txSender))) == sender || sender == address(this), ERR_SLOW_MODE_WRONG_SENDER);
    }
    ...
    function depositCollateralWithReferral(
        ...
        setReferralCode(sender, sender == msg.sender ? referralCode : DEFAULT_REFERRAL_CODE);
        ...
        uint64 executableAt = uint64(block.timestamp) + 30;
    ...
    function submitSlowModeTransaction(bytes calldata transaction) external {
        ...
        if (txType == TransactionType.DepositCollateral) {
            ...
            DepositInsurance memory txn = abi.decode(transaction[1:], (DepositInsurance));
    ...
    function _executeSlowModeTransaction(SlowModeConfig memory _slowModeConfig, bool fromSequencer) internal {
    ...
    function processSlowModeTransaction(address sender, bytes calldata transaction) public {
        ...
        if (txType == TransactionType.LiquidateSubaccount) {
            LiquidateSubaccount memory txn = abi.decode(transaction[1:], (LiquidateSubaccount));
            validateSender(txn.sender, sender);
            requireSubaccount(txn.sender);
            clearinghouse.liquidateSubaccount(txn);
        } else if (txType == TransactionType.DepositCollateral) {
            DepositCollateral memory txn = abi.decode(transaction[1:], (DepositCollateral));
            validateSender(txn.sender, sender);
            _recordSubaccount(txn.sender);
            clearinghouse.depositCollateral(txn);
        } else if (txType == TransactionType.WithdrawCollateral) {
            WithdrawCollateral memory txn = abi.decode(transaction[1:], (WithdrawCollateral));
            validateSender(txn.sender, sender);
            ...
            BurnLpAndTransfer memory txn = abi.decode(transaction[1:], (BurnLpAndTransfer));
            validateSender(txn.sender, sender);
    ...
    function processTransaction(bytes calldata transaction) internal {
        TransactionType txType = TransactionType(uint8(transaction[0]));
        if (txType == TransactionType.LiquidateSubaccount) {
            SignedLiquidateSubaccount memory signedTx = abi.decode(transaction[1:], (SignedLiquidateSubaccount));
            ...
            SignedWithdrawCollateral memory signedTx = abi.decode(transaction[1:], (SignedWithdrawCollateral));
            validateNonce(signedTx.tx.sender, signedTx.tx.nonce);
            bytes32 digest = _hashTypedDataV4(
                keccak256(
                    abi.encode(
            ...
            validateSignature(signedTx.tx.sender, digest, signedTx.signature);
            chargeFee(signedTx.tx.sender, spotEngine.getWithdrawFee(signedTx.tx.productId), signedTx.tx.productId);
            ...
            MatchOrderAMM memory txn = abi.decode(transaction[1:], (MatchOrderAMM));
            requireSubaccount(txn.taker.order.sender);
            IOffchainBook(books[txn.productId]).matchOrderAMM(txn, linkedSigners[txn.taker.order.sender]);
        } else if (txType == TransactionType.ExecuteSlowMode) {
            SlowModeConfig memory _slowModeConfig = slowModeConfig;
            _executeSlowModeTransaction(_slowModeConfig, true);
            slowModeConfig = _slowModeConfig;
        } else if (txType == TransactionType.MintLp) {
            SignedMintLp memory signedTx = abi.decode(transaction[1:], (SignedMintLp));
            validateNonce(signedTx.tx.sender, signedTx.tx.nonce);
            bytes32 digest = _hashTypedDataV4(
                keccak256(
                    abi.encode(
            ...
            SignedBurnLp memory signedTx = abi.decode(transaction[1:], (SignedBurnLp));
            validateNonce(signedTx.tx.sender, signedTx.tx.nonce);
            bytes32 digest = _hashTypedDataV4(
                keccak256(
                    abi.encode(
            ...
            ClaimSequencerFees memory txn = abi.decode(transaction[1:], (ClaimSequencerFees));
            uint32[] memory spotIds = spotEngine.getProductIds();
            ...
            ManualAssert memory txn = abi.decode(transaction[1:], (ManualAssert));
            ...
            SignedLinkSigner memory signedTx = abi.decode(transaction[1:], (SignedLinkSigner));
            validateNonce(signedTx.tx.sender, signedTx.tx.nonce);
            bytes32 digest = _hashTypedDataV4(
                keccak256(
                    abi.encode(
            ...
            UpdateFeeRates memory txn = abi.decode(transaction[1:], (UpdateFeeRates));
    ...
    function setBook(uint32 productId, address book) external {
        require(msg.sender == address(clearinghouse), ERR_ONLY_CLEARINGHOUSE_CAN_SET_BOOK);
    ...
    function getPriceX18(uint32 productId) public view returns (int128 priceX18) {
    ...
    function registerTransferableWallet(address wallet, bool transferable) external onlyOwner {
